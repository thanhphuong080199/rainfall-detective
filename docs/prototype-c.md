...
contradiction's claim role + constraint type — X, Y and Z's Prototype C
...
4. **Check Timeline** — disabled until every movable event is placed;
   ...
   On rejection, every placement is retained and the violated **required**
   facts (already-known sentences, never raw constraint data) are listed;
   identical resubmission is disabled until a real placement change is made.
...
5. **Final claim** — the disputed statement's own (translated) text is
   revealed, with two buttons: "Fits the timeline" / "Impossible". A wrong
   "Fits" answer keeps the accepted timeline exactly as built and offers a
   non-spoiling nudge; a correct "Impossible" answer reveals the
   contradiction explanation (with the player's own accepted time
   interpolated in) and offers Continue.
6. **Completion** — the authored `completion_text`, plus elapsed time,
   timeline checks, failed checks, placements/moves, facts opened, hints
   used, and claim-check attempts. Restart, Export Recording and Return to
...
- `can_resubmit()` is false immediately after any submission and stays
  false until a genuine placement change occurs — "identical failed
  submission is disabled until a move occurs." `submit_timeline()` itself
...
— `result` is whatever `PrototypeCController.submit_timeline()` returned:

| `result` shape | Player sees |
|---|---|
| `category == invalid_input` | A generic, static "place every event before checking" message (`UI_PROTOTYPE_C_FEEDBACK_INVALID`) |
| `category == timeline_inconsistent` | The natural-language text of every violated **required** fact (`result.violated_required`, filtered through `visible_constraint_facts` — an id with no fact mapping, which should never happen for well-formed content, is silently skipped rather than crashing), each paired with the opaque handles of the event(s) it references |
| `category == timeline_consistent` | `accepted: true`, no violations listed |

`violated_optional` (which, for well-formed content, is always exactly the
disputed claim's own constraint) is **never** read by the feedback builder —
the disputed claim stays completely invisible until the timeline is
accepted, exactly matching "do not reveal hidden or optional author-only
constraints." Feedback order is deterministic: `TimelineEvaluator.evaluate()`
walks `timeline.constraints` in authored JSON order, and the presenter never
re-sorts it.
...
and never state the final claim's answer.
...
established. Before acceptance, `view` has exactly 15 keys (`non_canon`,
...
| `claim_answered` | Every final-claim answer — payload: attempt number, "fits"/"impossible", whether it was correct |
| `contradiction_resolved` | Only on the first CORRECT "Impossible" answer |
...
`hint_ladder` has exactly 4 translated entries. The two heavier, bounded
